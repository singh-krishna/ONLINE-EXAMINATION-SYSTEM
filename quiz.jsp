            ...
            if (parseInt(sec) > 0) 
               { sec = parseInt(sec) - 1;
                ...
                if (parseInt(sec) == 0) {
                    ...
                    if (parseInt(min) == -1) {
                        clearTimeout(tim);
						var r=alert("Time's up");
						
						setTimeout(function(){window.location.href='validate.jsp'},100);
					}
...
<%
    ...
    Connection con = DriverManager.getConnection("jdbc:mysql://localhost/krishna","root","");
	Statement st = con.createStatement();
	ResultSet rs=st.executeQuery("select * from exam"); 
	while(rs.next()){
%>
<div class="ques">
<form action="validate.jsp" method="post" id="abc">
<div class="test">
<tr>
<th><h4><%=rs.getString(1)%>. <%=rs.getString(2)%></h4></th>
<tr>
<td><p><input type="radio" name= "x<%=Integer.toString(x)%>" value="1"><%=rs.getString(3)%><br></td>
<td><input type="radio" name= "x<%=Integer.toString(x)%>" value="2"><%=rs.getString(4)%><br></td>
<td><input type="radio" name= "x<%=Integer.toString(x)%>" value="3"><%=rs.getString(5)%><br></td>
<td><input type="radio" name= "x<%=Integer.toString(x)%>" value="4"><%=rs.getString(6)%></p><br></td></tr>
<%
x++;
%>
</div>
