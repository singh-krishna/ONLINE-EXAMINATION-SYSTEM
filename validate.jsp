...
<%
	...
	while(rs.next()){
        count++;
        arr[s]=(Integer.toString(rs.getInt(7)));
        s++;
    }
    //out.println("counter value:"+count);
    for(int i=1;i<=count;i++)
    {
        if(request.getParameter("x"+Integer.toString(i))!=null)
            a[i]=request.getParameter("x"+Integer.toString(i));
        else 
            a[i]="0";
        //out.println(a[i]);
    }
   ...
   /* while(rs.next())
    ...
    <div class="show">
        <p> View Answers </p>
        <a href="display.jsp"><input type="button" value="Click"></type="button"></a>
        <a href="start.html"><button class="btn success" style="float: right;">Logout</button></a>
    </div>
    <%
    }
